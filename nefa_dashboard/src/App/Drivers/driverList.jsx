...
const Drivers = ({ setComponentName }) => {
    ...
    const data = [
        {
            name: "Amit",
            email: "amit.sharma@cotgin.com",
            mobile: "9865865785",
            address: "22,tilak appt. surat",
            joiningData: "22 Feb 2020",
            trips: 212,
            status: "Available",
        },
        {
    ...
    const columns = [
        {
            ...
            cell: ({ row }) => (
                <td>
                    <img src={UserProfile} className="pt-1 rounded-full h-[45px] w-[45px] object-contain" />
                </td>
            )
        },
        {
        ...
        },
        {
            ...
            accessorKey: "status",
            cell: ({ row }) => (
                <span className={`${row?.original?.status === "Available" ? "text-[#fcad47]" : row?.original?.status === "Leave" ? "text-[#ed6473]" : "text-[#49c1b7]"} font-medium`}>{row?.original?.status} </span>
            ),
        },
        {
            header: "Actions",
